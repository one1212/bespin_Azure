...
public class NineBlockIdenticonRenderer implements IdenticonRenderer {
    ...
    public void setPatchSize(float size) {
        ...
        // correctly.
        float patchOffset = patchSize / 2.0f; // used to center patch shape at
        ...
        for (int i = 0; i < patchTypes.length; i++) {
            GeneralPath patch = getGeneralPath(i, patchScale, patchOffset);
            patch.closePath();
            this.patchShapes[i] = patch;
        }
    }

    private static GeneralPath getGeneralPath(int i, float patchScale, float patchOffset) {
        GeneralPath patch = new GeneralPath(GeneralPath.WIND_NON_ZERO);

        boolean moveTo = true;
        byte[] patchVertices = patchTypes[i];

        for (byte patchVertex : patchVertices) {
            if ((int) patchVertex == PATCH_MOVETO) {
                moveTo = true;
            }

            float vx = (((int) patchVertex % PATCH_GRIDS) * patchScale) - patchOffset;
            float vy = ((float) Math.floor(((float) (int) patchVertex) / PATCH_GRIDS)) * patchScale - patchOffset;
            if (!moveTo) {
                patch.lineTo(vx, vy);
            } else {
                moveTo = false;
